...
void lcd_writecommand(unsigned char cmd)
{
    ...
    unsigned char up_bits = 0x00;
    unsigned char low_bits = 0x00;


    PORTB &= 0b11111110;  //CLEAR PB0 to 0 

    up_bits = (cmd & DATA_BITS);
    lcd_writenibble(up_bits);

    low_bits = (cmd & (~DATA_BITS));
    low_bits = low_bits <<4; 
    lcd_writenibble(low_bits);
...
{
    unsigned char upper_bits = 0x00; 
    unsigned char lower_bits = 0x00; 
    PORTB |= (1<<PB0);

    upper_bits = (dat & DATA_BITS);
    lcd_writenibble(upper_bits);

    lower_bits = (dat & (~DATA_BITS));
    lower_bits = lower_bits <<4; 
    lcd_writenibble(lower_bits);
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    PORTD &= ~(DATA_BITS);    //Clear bits 7-4 with inverted mask
    PORTD |= (lcdbits);       //Set bits 7-4 with lcdbits 
